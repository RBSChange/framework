...
class validation_ContraintsParser
{
	...
	private function parseDefinition($string)
	{
		...
		for ($i = 0 ; $i < $length ; $i++)
		{
			...
			{
				throw new Exception("Malformed validator definition: unexpected character '".$string{$i}."' at position ".$i);
			}
		}
	}
	...
	private function buildValidatorFromConstraint($name, $value)
	{
		...
		{
			throw new Exception("Unknown validator: ".$className);
		}
		
		$validator = new $className();
		if ( ! $validator instanceof validation_Validator )
		{
			throw new Exception("Invalid validator: ".$className."; must be an instance of 'validation_Validator'.");
		}
