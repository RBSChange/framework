...
class Browscap
{
	 ...
	 * @var string
	 */
	public $cacheFilename 	= 'cache_browscap.ini.php';
	...
	public function getBrowser($user_agent = null, $return_array = false)
	{
		...
		foreach ($this->_patterns as $key => $pattern) 
		{
			...
			{
				$browser = array(trim(strtolower($pattern), self::REGEX_DELIMITER), $this->_userAgents[$key]);
	...
	public function getGraphicalEngines()
	{
		if (!$this->_cacheLoaded) {
			$cache_file = $this->cacheDir . $this->cacheFilename;
		...
		$ek = array_search('GraphicalEngine', $this->_properties, true);
		$ekv = array_search('GraphicalEngineVer', $this->_properties, true);
		$results = array();
		foreach ($this->_browsers as $value)
		{
			$browser = $value;
			while (array_key_exists(2, $value))
			{
	...
	public function updateCache()
	{
		...
		//TODO inthause : on ne telecharge pas le browsercap
		...
		//$this->_userAgents = array_reverse($this->_userAgents);
