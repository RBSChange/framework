 ...
 * @copyright (c) Authors
 * @since     0.9.1
 * @version   $Id: LoggingConfigHandler.class.php 165 2005-07-06 09:11:13Z bob $
 */
class ChangeLoggingConfigHandler extends IniConfigHandler
{
	...
	public function & execute ($config)
	{
		...
		$categories = array('required_categories' => array('loggers','loggerGroups'));
		...
		$this->loadLoggerGroups($loggerGroups, $ini);
		$this->generateRegistration($data, $loggers, $appenders, $layouts,$loggerGroups);
	 ...
	 * @since  0.9.0
	 */
	private function generateRegistration(&$data, &$loggers, &$appenders, &$layouts,&$loggerGroups)
	{
		...
		*/
		...
		foreach (array_keys($loggerGroups) as $loggerGroupName)
		{
			$str = 'LoggerManager::setLoggerGroup("%s", array( \'%s\' ));';
			$data[] = sprintf($str, strtolower($loggerGroupName),implode("','",$loggerGroups[$loggerGroupName]));
		}
	...
	private function loadAppenders(&$config, &$loggers, &$appenders, &$layouts, &$ini)
	{
		...
		foreach (array_keys($appenders) as $appender) {
			...
			//webedit bug $appenders[$appender] = &$entry;
	...
	private function loadLoggerGroups(&$LoggerGroups,&$ini)
	{
		 foreach ($ini['loggerGroups'] as $key => &$value) {
			// explode the list of names
			$LoggerGroups[trim($key)]= explode(',', str_replace(" ","",$value));
		 }
	}
