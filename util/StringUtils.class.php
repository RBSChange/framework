...
class f_util_StringUtils
{
	...
	final static function transcodeStringsInArray($in_array = null)
	{
		...
		if ($in_array != null && count($in_array) > 0)
		{
			...
			for ($i = 0; $i < $l_nbElementsInArray; $i++)
			{
				...
				if (is_array($l_element)) $in_array[$l_keys[$i]] = self::transcodeStringsInArray($l_element);
				else $in_array[$l_keys[$i]] = self::transcodeString($in_array[$l_keys[$i]]);
			}
		}
	...
	public static function transcodeString($in_toTranscode = null)
	{
		...
		{
			if (mb_strpos($in_toTranscode, "%u") === false) $l_result = $in_toTranscode;
			else
			{
		...
		if (mb_strlen($l_result) == 0) $l_result = $in_toTranscode;
	...
	{
		$randomString = "";
		$consons = array("b", "c", "d", "f", "g", "h", "j", "k", "l", "m", "n", "p", "q", "r", "s", "t", "v", "z", "bl", "br", "cl", "cr", "ch", "dr", 
			"fl", "fr", "gl", "gr", "pl", "pr", "qu", "sl", "sr");
	...
	public static function isHexa($in_hexaTest = null)
	{
		...
		{
			if (!in_array($l_value[$j], $l_allowed)) return false;
		}
	...
	public static function prefixUpper($string, $nbLetter = 1, $forceLower = false)
	{
		...
		{
			if ($forceLower == true) $lower = mb_strtolower(mb_substr($string, $nbLetter, strlen($string)));
			else $lower = mb_substr($string, $nbLetter, strlen($string));
		}
		return ($upper . $lower);
	}
